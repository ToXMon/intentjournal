...
export default function RealCrossChainIntentDemo() {
  ...
  const checkBalances = async () => {
    ...
    try {
      // Simulate checking balances (already done via wagmi hooks)
  ...
  const createIntent = async () => {
    ...
    try {
      ...
      const intentData = {
        tokenIn: BUILDBEAR_TOKENS.USDC,
        tokenOut: BUILDBEAR_TOKENS.BBETH,
        amountIn: parseUnits(swapAmount, 6), // USDC has 6 decimals
        minAmountOut: parseUnits('0.01', 18), // Minimum BBETH out
        deadline: BigInt(Math.floor(Date.now() / 1000) + 3600), // 1 hour
        crossChain: true,
        targetChain: 128123 // Etherlink testnet
      };
      
      // Simulate intent creation (replace with actual contract call)
